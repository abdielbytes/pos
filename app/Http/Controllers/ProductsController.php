    ...
    public function index(request $request)
    {
        ...
        foreach ($products as $product) {
            ...
            if($product->type == 'is_single'){
              ...
              $product_branch_total_qty = product_branch::where('product_id', $product->id)
              ->where('deleted_at', '=', null)
              ->sum('qte');

              $item['quantity'] = $product_branch_total_qty .' '.$product['unit']->ShortName;
...
//                'tax_method'   => 'required',
           ...
           // if type is not is_variant, add validation for variants array
            ...
            \DB::transaction(function () use ($request) {
               ...
               if ($request['type'] == 'is_variant') {
                ...
                //--Store Product branch
                $branches = branch::where('deleted_at', null)->pluck('id')->toArray();
                if ($branches) {
                ...
                if ($request['type'] == 'is_variant') {
                    ...
                    if ($oldVariants->isNotEmpty()) {
                        ...
                        foreach ($oldVariants as $key => $value) {
                            ...
                            if (!in_array($old_variants_id[$key], $new_variants_id)) {
                                ...
                                $Productbranch = product_branch::where('product_variant_id', $value->id)
                        ...
                        $Producttbranch = product_branch::where('product_id', $id)
                            ->update([
                        ...
                        foreach ($request['variants'] as $variant) {
                           ...
                           $ProductVarDT->save();
                            ...
                            if ($branches) {
                                ...
                                product_branch::insert($product_branch_DT);
                            }
                        }
                    ...
                    if ($oldVariants->isNotEmpty()) {
                        foreach ($oldVariants as $old_var) {
                            ...
                            $Producttbranch = product_branch::where('product_variant_id', $old_var['id'])
                                ->update([
                                    'deleted_at' => Carbon::now(),
                                ]);
                        }
    ...
    public function destroy(Request $request, $id)
    {
        ...
        \DB::transaction(function () use ($id) {
            ...
            product_branch::where('product_id', $id)->update([
                'deleted_at' => Carbon::now(),
            ]);
    ...
    public function delete_by_selection(Request $request)
    {
        ...
        \DB::transaction(function () use ($request) {
            ...
            foreach ($selectedIds as $product_id) {
                ...
                product_branch::where('product_id', $product_id)->update([
                    'deleted_at' => Carbon::now(),
                ]);
    ...
    public function Get_Products_Details(Request $request, $id)
    {
        ...
        if ($Product->is_variant) {
            ...
            foreach ($productsVariants as $variant) {
                ...
                foreach ($branches as $branch) {
                    $product_branch = DB::table('product_branch')
                        ->where('product_id', $id)
                        ->where('deleted_at', '=', null)
                        ->where('branch_id', $branch->id)
                        ->where('product_variant_id', $variant->id)
                        ->select(DB::raw('SUM(product_branch.qte) AS sum'))
                        ->first();

                    $war_var['mag'] = $branch->name;
                    $war_var['variant'] = $variant->name;
                    $war_var['qte'] = $product_branch->sum;
        ...
        foreach ($branches as $branch) {
            $product_branch_data = DB::table('product_branch')
                ->where('deleted_at', '=', null)
                ->where('product_id', $id)
                ->where('branch_id', $branch->id)
                ->select(DB::raw('SUM(product_branch.qte) AS sum'))
            ...
            $war['mag'] = $branch->name;
            $war['qte'] = $product_branch_data->sum;
            $item['CountQTY'][] = $war;
        }
    ...
    public function Products_by_branch(request $request, $id)
    {
        $data = [];
        ...
        foreach ($product_branch_data as $product_branch) {
            ...
            if ($product_branch->product_variant_id) {
                ...
                $item['code'] = $product_branch['productVariant']->code;
                $item['Variant'] = '['.$product_branch['productVariant']->name . ']' . $product_branch['product']->name;
                $item['name'] = '['.$product_branch['productVariant']->name . ']' . $product_branch['product']->name;
                $item['barcode'] = $product_branch['productVariant']->code;
            ...
            $item['Type_barcode'] = $product_branch['product']->Type_barcode;
            ...
            if($product_branch['product']['unitPurchase']) {

                if ($product_branch['product']['unitPurchase']->operator == '/') {
                    $item['qte_purchase'] = round($product_branch->qte * $product_branch['product']['unitPurchase']->operator_value, 5);
                } else {
                    $item['qte_purchase'] = round($product_branch->qte / $product_branch['product']['unitPurchase']->operator_value, 5);
                }

            }else{
                $item['qte_purchase'] = $product_branch->qte;
            }

            $item['manage_stock'] = $product_branch->manage_stock;
            ...
            $item['unitPurchase'] = $product_branch['product']['unitPurchase']?$product_branch['product']['unitPurchase']->ShortName:'';
    ...
    public function Products_Alert(request $request)
    {
        ...
        $product_branch_data = product_branch::with('branch', 'product', 'productVariant')
            ...
            ->where(function ($query) use ($request) {
                return $query->when($request->filled('branch'), function ($query) use ($request) {
                    return $query->where('branch_id', $request->branch);
                });
            })->where('product_branch.deleted_at', null)->get();
    ...
    public function import_products(Request $request)
    {
        ...
        if ($ext != 'csv') {
            ...
            try {
                \DB::transaction(function () use ($data , $branches) {
