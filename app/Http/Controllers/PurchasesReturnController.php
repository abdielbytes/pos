...
class PurchasesReturnController extends BaseController
{
    ...
    public function index(request $request)
    {
        ...
        $PurchaseReturn = PurchaseReturn::with('purchase','facture', 'provider', 'branch')
                ...
                if($value['purchase_unit_id'] !== null){
                    if ($current_PurchaseReturn->statut == "completed") {
                        if ($value['product_variant_id'] !== null) {
                            ...
                            $product_branch = product_branch::where('deleted_at', '=', null)
                                ->where('branch_id', $current_PurchaseReturn->branch_id)
                                ->where('product_id', $value['product_id'])
                                ->where('product_variant_id', $value['product_variant_id'])
                                ->first();
    ...
    public function edit_purchase_return(Request $request, $id, $purchase_id)
    {
        ...
        $Return_detail['branch_id'] = $Purchase_Return->branch_id;
        ...
        foreach ($Purchase_Return['details'] as $detail) {
            ...
            if ($detail->product_variant_id) {
                $item_product = product_branch::where('product_id', $detail->product_id)
                    ->where('deleted_at', '=', null)
                    ->where('product_variant_id', $detail->product_variant_id)
                    ->where('branch_id', $Purchase_Return->branch_id)
                    ->first();
    ...
    public function show(Request $request, $id)
    {
        ...
        $return_details['branch'] = $Purchase_Return['branch']->name;
