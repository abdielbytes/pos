...
class ReportController extends BaseController
{
    ...
    public function Sales_Client(request $request)
    {
        ...
        $sales = Sale::where('deleted_at', '=', null)->with('client','branch')
            ->where(function ($query) use ($ShowRecord) {
                if (!$ShowRecord) {
                    return $query->where('user_id', '=', Auth::user()->id);
                }
        ...
        foreach ($sales as $sale) {
            ...
            $item['date'] = $sale->date;
            $item['Ref'] = $sale->Ref;
            $item['branch_name'] = $sale['branch']->name;
            $item['client_name'] = $sale['client']->name;
            $item['statut'] = $sale->statut;
            $item['GrandTotal'] = $sale->GrandTotal;
            $item['paid_amount'] = $sale->paid_amount;
    ...
    public function Quotations_Client(request $request)
    {
        ...
        $Quotations = Quotation::with('client', 'branch')
            ->where('deleted_at', '=', null)
            ->where('client_id', $request->id)
            ->where(function ($query) use ($ShowRecord) {
                if (!$ShowRecord) {
        ...
        foreach ($Quotations as $Quotation) {
            ...
            $item['branch_name'] = $Quotation['branch']->name;
    ...
    public function Returns_Client(request $request)
    {
        ...
        $SaleReturn = SaleReturn::where('deleted_at', '=', null)->with('sale','client','branch')
            ->where('client_id', $request->id)
            ->where(function ($query) use ($ShowRecord) {
                if (!$ShowRecord) {
                    return $query->where('user_id', '=', Auth::user()->id);
        ...
        foreach ($SaleReturn as $Sale_Return) {
            ...
            $item['branch_name'] = $Sale_Return['branch']->name;
    ...
    public function Returns_Provider(request $request)
    {
        ...
        foreach ($PurchaseReturn as $Purchase_Return) {
            ...
            $item['branch_name'] = $Purchase_Return['branch']->name;
    ...
    public function branch_Report(request $request)
    {
        ...
        $this->authorizeForUser($request->user('api'), 'branchestock', Product::class);
    ...
    public function Quotations_branch(request $request)
    {
    ...
    //-------------------- Get Returns Sale By branch -------------\\

    public function Returns_Sale_branch(request $request)
    {
    ...
    //-------------------- Get Returns Purchase By branch -------------\\

    public function Returns_Purchase_branch(request $request)
    {
    ...
    public function Expenses_branch(request $request)
    {
        ...
        foreach ($Expenses as $Expense) {
            ...
            $item['branch_name'] = $Expense['branch']->name;
    ...
    public function Warhouse_Count_Stock(Request $request)
    {
        ...
        $stock_count = product_branch::join('products', 'product_branch.product_id', '=', 'products.id')
            ->join('branches', 'product_branch.branch_id', '=', 'branches.id')
            ->where('product_branch.deleted_at', '=', null)
            ->select(
                DB::raw("count(DISTINCT products.id) as value"),
                DB::raw("branches.name as name"),
        ...
        $stock_value = product_branch::join('products', 'product_branch.product_id', '=', 'products.id')
            ->join('branches', 'product_branch.branch_id', '=', 'branches.id')
            ->where('product_branch.deleted_at', '=', null)
            ->select(
    ...
    public function count_quantity_alert(request $request)
    {
     ...
     public function ProfitAndLoss(request $request)
     {
         ...
         ->where(function ($query) use ($request, $branch_id, $array_branches_id) {
             if ($branch_id !== 0) {
         ...
         ->where(function ($query) use ($request, $branch_id, $array_branches_id) {
             if ($branch_id !== 0) {
                 return $query->whereHas('sale', function ($q) use ($request, $array_branches_id, $branch_id) {
         ...
         ->whereBetween('date', array($start_date, $end_date))
             ...
             ->where(function ($query) use ($request, $branch_id, $array_branches_id) {
                 if ($branch_id !== 0) {
                     return $query->whereHas('SaleReturn', function ($q) use ($request, $array_branches_id, $branch_id) {
                         $q->where('branch_id', $branch_id);
                     });
                 }else{
                     return $query->whereHas('SaleReturn', function ($q) use ($request, $array_branches_id, $branch_id) {
        ...
        //--------PaymentPurchaseReturns
         ...
         ->where(function ($query) use ($request, $branch_id, $array_branches_id) {
             if ($branch_id !== 0) {
                 return $query->whereHas('PurchaseReturn', function ($q) use ($request, $array_branches_id, $branch_id) {
                     $q->where('branch_id', $branch_id);
                 });
             }else{
                 return $query->whereHas('PurchaseReturn', function ($q) use ($request, $array_branches_id, $branch_id) {
         ...
         ->where(function ($query) use ($request, $branch_id, $array_branches_id) {
             if ($branch_id !== 0) {
                 return $query->whereHas('purchase', function ($q) use ($request, $array_branches_id, $branch_id) {
                     $q->where('branch_id', $branch_id);
                 });
             }else{
                 return $query->whereHas('purchase', function ($q) use ($request, $array_branches_id, $branch_id) {
      ...
      //calcule COGS and average cost
         $cogs_average_data = $this->CalculeCogsAndAverageCost($start_date, $end_date, $branch_id, $array_branches_id);
     ...
     public function CalculeCogsAndAverageCost($start_date, $end_date , $branch_id, $array_branches_id)
     {
        ...
        // Get all distinct product IDs for sales between start and end date
         ...
         ->where(function ($query) use ($branch_id, $array_branches_id) {
             if ($branch_id !== 0) {
                 return $query->whereHas('sale', function ($q) use ($array_branches_id, $branch_id) {
                     $q->where('branch_id', $branch_id)->where('statut', 'completed');
                 });
             }else{
                 return $query->whereHas('sale', function ($q) use ($array_branches_id, $branch_id) {
                     $q->whereIn('branch_id', $array_branches_id)->where('statut', 'completed');
                 });
         ...
         foreach ($productIds as $productId) {
             ...
             ->where(function ($query) use ($branch_id, $array_branches_id ,$end_date) {
                 if ($branch_id !== 0) {
                     return $query->whereHas('adjustment', function ($q) use ($array_branches_id, $branch_id,$end_date) {
                         $q->where('branch_id', $branch_id)
                         ->where('date', '<=' , $end_date);
                     });
                 }else{
             ...
             ->sum('quantity');
              ...
              ->where(function ($query) use ($branch_id, $array_branches_id) {
                 if ($branch_id !== 0) {
                     return  $query->where('purchases.branch_id', $branch_id)->where('purchases.statut', 'received');
                 }else{
                     return  $query->whereIn('purchases.branch_id', $array_branches_id)->where('purchases.statut', 'received');
             ...
             $average_cost = $this->averageCost($productId['product_id'] ,$start_date, $end_date, $branch_id, $array_branches_id);
     ...
     public function averageCost($product_id , $start_date, $end_date , $branch_id, $array_branches_id)
     {
         ...
         // Get the total cost and quantity for all adjustments of the product
         $adjustments = AdjustmentDetail::with('adjustment')
         ->where(function ($query) use ($branch_id, $array_branches_id, $start_date, $end_date) {
             if ($branch_id !== 0) {
                 return $query->whereHas('adjustment', function ($q) use ($array_branches_id, $branch_id, $start_date, $end_date) {
                     $q->where('branch_id', $branch_id)
                     ->where('date', '<=' , $end_date);
                 });
             }else{
    ...
    public function get_sales_by_user(request $request)
    {
        ...
        $sales = Sale::where('deleted_at', '=', null)->with('user','client','branch')
            ->where(function ($query) use ($ShowRecord) {
                if (!$ShowRecord) {
                    return $query->where('user_id', '=', Auth::user()->id);
                }
     ...
     public function get_quotations_by_user(request $request)
     {
        ...
        $this->authorizeForUser($request->user('api'), 'users_report', User::class);
         ...
         $Quotations = Quotation::with('client', 'branch','user')
            ->where('deleted_at', '=', null)
             ->where('user_id', $request->id)
             ->where(function ($query) use ($ShowRecord) {
                 if (!$ShowRecord) {
    ...
    public function get_purchases_by_user(request $request)
    {
     ...
     public function get_sales_return_by_user(request $request)
     {
        ...
        $this->authorizeForUser($request->user('api'), 'users_report', User::class);
         ...
         $SaleReturn = SaleReturn::where('deleted_at', '=', null)->with('user','client','branch')
             ->where('user_id', $request->id)
             ->where(function ($query) use ($ShowRecord) {
                 if (!$ShowRecord) {
                     return $query->where('user_id', '=', Auth::user()->id);
    ...
    public function stock_Report(request $request)
    {
        ...
        if($user_auth->is_all_branches){
            $branches = branch::where('deleted_at', '=', null)->get(['id', 'name']);
            $branches_id = branch::where('deleted_at', '=', null)->pluck('id')->toArray();
        }else{
            $branches_id = Userbranch::where('user_id', $user_auth->id)->pluck('branch_id')->toArray();
            $branches = branch::where('deleted_at', '=', null)->whereIn('id', $branches_id)->get(['id', 'name']);
        }
        ...
        foreach ($products as $product) {
            ...
            if($product->type != 'is_service'){
                ...
                $current_stock = product_branch::where('product_id', $product->id)
                ->where('deleted_at', '=', null)
    ...
    public function get_sales_by_product(request $request)
    {
        ...
        $sale_details_data = SaleDetail::with('product','sale','sale.client','sale.branch')
            ->where(function ($query) use ($ShowRecord) {
                if (!$ShowRecord) {
                    return $query->whereHas('sale', function ($q) use ($request) {
             ...
             ->where(function ($query) use ($request) {
                return $query->when($request->filled('search'), function ($query) use ($request) {
                    return $query->where(function ($query) use ($request) {
                        ...
                        ->orWhere(function ($query) use ($request) {
                            return $query->whereHas('sale.branch', function ($q) use ($request) {
                                $q->where('name', 'LIKE', "%{$request->search}%");
                            });
                        })
                        ->orWhere(function ($query) use ($request) {
        ...
        foreach ($sale_details as $detail) {
            ...
            $item['branch_name'] = $detail['sale']['branch']->name;
    ...
    public function get_quotations_by_product(request $request)
    {
        ...
        $quotation_details_data = QuotationDetail::with('product','quotation','quotation.client','quotation.branch')
            ->where(function ($query) use ($ShowRecord) {
                if (!$ShowRecord) {
            ...
            ->where(function ($query) use ($request) {
                return $query->when($request->filled('search'), function ($query) use ($request) {
                    return $query->where(function ($query) use ($request) {
                        ...
                        ->orWhere(function ($query) use ($request) {
                            return $query->whereHas('quotation.branch', function ($q) use ($request) {
                                $q->where('name', 'LIKE', "%{$request->search}%");
                            });
                        })
                        ->orWhere(function ($query) use ($request) {
        ...
        foreach ($quotation_details as $detail) {
            ...
            $item['branch_name'] = $detail['quotation']['branch']->name;
            $item['unit_sale'] = $unit?$unit->ShortName:'';
            $item['quantity'] = $detail->quantity .' '.$item['unit_sale'];
            $item['total'] = $detail->total;
            $item['product_name'] = $product_name;
    ...
    public function get_purchases_by_product(request $request)
    {
        ...
        $purchase_details_data = PurchaseDetail::with('product','purchase','purchase.provider','purchase.branch')
            ->where(function ($query) use ($ShowRecord) {
                if (!$ShowRecord) {
                    return $query->whereHas('purchase', function ($q) use ($request) {
                        $q->where('user_id', '=', Auth::user()->id);
            ...
            ->where(function ($query) use ($request) {
                return $query->when($request->filled('search'), function ($query) use ($request) {
                    return $query->where(function ($query) use ($request) {
                        ...
                        ->orWhere(function ($query) use ($request) {
                            return $query->whereHas('purchase.branch', function ($q) use ($request) {
                                $q->where('name', 'LIKE', "%{$request->search}%");
                            });
                        })
                        ->orWhere(function ($query) use ($request) {
        ...
        foreach ($purchase_details as $detail) {
            ...
            $item['branch_name'] = $detail['purchase']['branch']->name;
    ...
    public function get_purchase_return_by_product(request $request)
    {
        ...
        $purchase_return_details_data = PurchaseReturnDetails::with('product','PurchaseReturn','PurchaseReturn.provider','PurchaseReturn.branch')
            ->where(function ($query) use ($ShowRecord) {
                if (!$ShowRecord) {
            ...
            ->where(function ($query) use ($request) {
                return $query->when($request->filled('search'), function ($query) use ($request) {
                    return $query->where(function ($query) use ($request) {
                        ...
                        ->orWhere(function ($query) use ($request) {
                            return $query->whereHas('PurchaseReturn.branch', function ($q) use ($request) {
                                $q->where('name', 'LIKE', "%{$request->search}%");
                            });
                        })
                        ->orWhere(function ($query) use ($request) {
            ...
            $item['branch_name'] = $detail['PurchaseReturn']['branch']->name;
            $item['quantity'] = $detail->quantity .' '.$unit->ShortName;;
            $item['total'] = $detail->total;
            $item['product_name'] = $product_name;
            $item['unit_purchase'] = $unit->ShortName;
    ...
    public function get_sales_return_by_product(request $request)
    {
        ...
        $Sale_Return_details_data = SaleReturnDetails::with('product','SaleReturn','SaleReturn.client','SaleReturn.branch')
            ->where(function ($query) use ($ShowRecord) {
                if (!$ShowRecord) {
            ...
            ->where(function ($query) use ($request) {
                return $query->when($request->filled('search'), function ($query) use ($request) {
                    return $query->where(function ($query) use ($request) {
                        ...
                        ->orWhere(function ($query) use ($request) {
                            return $query->whereHas('SaleReturn.branch', function ($q) use ($request) {
                                $q->where('name', 'LIKE', "%{$request->search}%");
                            });
                        })
                        ->orWhere(function ($query) use ($request) {
        ...
        foreach ($Sale_Return_details as $detail) {
            ...
            $item['branch_name'] = $detail['SaleReturn']['branch']->name;
            $item['unit_sale'] = $unit?$unit->ShortName:'';
            $item['quantity'] = $detail->quantity .' '.$item['unit_sale'];
            $item['total'] = $detail->total;
            $item['product_name'] = $product_name;
    ...
    public function get_transfer_by_product(request $request)
    {
        ...
        $transfer_details_data = TransferDetail::with('product','transfer','transfer.from_branch','transfer.to_branch')
            ->where(function ($query) use ($ShowRecord) {
                if (!$ShowRecord) {
            ...
            ->where(function ($query) use ($request) {
                return $query->when($request->filled('search'), function ($query) use ($request) {
                    return $query->where(function ($query) use ($request) {
                            return $query->whereHas('transfer.from_branch', function ($q) use ($request) {
                                $q->where('name', 'LIKE', "%{$request->search}%");
                            });
                        })
                        ->orWhere(function ($query) use ($request) {
                            return $query->whereHas('transfer.to_branch', function ($q) use ($request) {
                                $q->where('name', 'LIKE', "%{$request->search}%");
                            });
                        })
                        ->orWhere(function ($query) use ($request) {
        ...
        foreach ($transfer_details as $detail) {
            ...
            $item['from_branch'] = $detail['transfer']['from_branch']->name;
            $item['to_branch'] = $detail['transfer']['to_branch']->name;
            $item['product_name'] = $product_name;
    ...
    public function get_adjustment_by_product(request $request)
    {
        ...
        $adjustment_details_data = AdjustmentDetail::with('product','adjustment','adjustment.branch')
            ->where(function ($query) use ($ShowRecord) {
                if (!$ShowRecord) {
            ...
            ->where('product_id', $request->id)
                ...
                return $query->when($request->filled('search'), function ($query) use ($request) {
                    return $query->where(function ($query) use ($request) {
                            return $query->whereHas('adjustment.branch', function ($q) use ($request) {
                                $q->where('name', 'LIKE', "%{$request->search}%");
                            });
                        })
                        ->orWhere(function ($query) use ($request) {
        ...
        foreach ($adjustment_details as $detail) {
            ...
            $item['branch_name'] = $detail['adjustment']['branch']->name;
            $item['product_name'] = $product_name;
    ...
    public function product_report(request $request)
    {
        ...
        foreach ($products as $product) {
            ...
            if($product->type != 'is_service'){
                ...
                ->where(function ($query) use ($request, $array_branches_id) {
                    if ($request->branch_id) {
                        return $query->whereHas('sale', function ($q) use ($request, $array_branches_id) {
    ...
    public function sale_products_details(request $request)
    {
        ...
        $sale_details_data = SaleDetail::with('product','sale','sale.client','sale.branch','sale.user')
            ->where(function ($query) use ($ShowRecord) {
                if (!$ShowRecord) {
                    return $query->whereHas('sale', function ($q) use ($request) {
                        $q->where('user_id', '=', Auth::user()->id);
