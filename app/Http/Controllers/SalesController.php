...
class SalesController extends BaseController
{
    ...
    public function index(Request $request)
    {
        ...
        $Sales = Sale::with(['facture', 'client', 'branch', 'user', 'details'])
        ...
        foreach ($Sales as $Sale) {
            $hasPaid = $Sale->checkAndApplyPenalty($Sale->payment_frequency);
            ...
            $item['penalty'] = $Sale['penalty'];
            // $item['installment'] = $Sale['installment'];
            $item['installment'] = installments::pluck('installment');
            // $item['installment'] = DB::table('installments')->where('id', $id)->value('amount'); // Replace 'amount' with your column name

            $item['GrandTotal'] = number_format($Sale['GrandTotal'] + $item['penalty'], 2, '.', '');
            $item['paid_amount'] = number_format($Sale['paid_amount'], 2, '.', '');
            $item['penalty'] = number_format($Sale['penalty'], 2, '.', '');
            $item['due'] = number_format($item['GrandTotal'] - $item['paid_amount'], 2, '.', '');
            $item['payment_status'] = $Sale['payment_statut'];
            $item['payment_frequency'] = $Sale['payment_frequency'];
            $item['has_paid'] = $hasPaid;
            $item['sale_details'] = [];
            foreach ($Sale->details as $detail) {
                $item['sale_details'][] = [
                    'payment_frequency' => $detail->payment_frequency,
    ...
    public function store(Request $request)
{
    ...
    $validated = $request->validate([
        ...
        // Add validation for payment_frequency
//        'payment_frequency' => 'required_if:payment.status,!=,pending|in:daily,weekly'
    ]);
    ...
    \DB::transaction(function () use ($request) {
        ...
        $order->installment = installments::pluck('installment');
            ...
            try {
                ...
                if ($request['amount'] > 0) {
                    if ($request->payment['Reglement'] == 'credit card') {
                        ...
                        $PaymentSale->payment_frequency = $request->payment_frequency; // Save payment frequency
                        $PaymentSale->save();
...
//                            'payment_frequency' => $request->payment_frequency // Save payment frequency
                        ]);
            ...
            if (SaleReturn::where('sale_id', $id)->where('deleted_at', '=', null)->exists()) {
                ...
                foreach ($old_sale_details as $key => $value) {
                    ...
                    if ($current_Sale->statut == "completed") {
                        ...
                        if ($value['product_variant_id'] !== null) {
                            $product_branch = product_branch::where('deleted_at', '=', null)
                                ->where('branch_id', $current_Sale->branch_id)
                                ->where('product_id', $value['product_id'])
                                ->where('product_variant_id', $value['product_variant_id'])
                                ->first();

                            if ($product_branch && $old_unit) {
                                if ($old_unit->operator == '/') {
                                    $product_branch->qte += $value['quantity'] / $old_unit->operator_value;
                                } else {
                                    $product_branch->qte += $value['quantity'] * $old_unit->operator_value;
                                }
                                $product_branch->save();
    ...
    public function show(Request $request, $id)
    {
        ...
        $sale_details['branch'] = $sale_data['branch']->name;
        ...
        $sale_details['penalty'] = $sale_data['penalty'];
        $sale_details['GrandTotal'] = number_format($sale_data->GrandTotal + $sale_data->penalty, 2, '.', '');
        $sale_details['penalty'] = number_format($sale_data['penalty']);
...
//        $sale_details['payment_frequency'] = $sale_data['payment_frequency'];
//        $sale_details['due'] = number_format($sale_details['GrandTotal'] - $sale_details['paid_amount'] + $sale_details['penalty'], 2, '.', '');
        $sale_details['payment_status'] = $sale_data->payment_statut;
        $sale_details['sales_details'] = $sale_data->details;
      ...
      public function edit(Request $request, $id)
      {
        if (SaleReturn::where('sale_id', $id)->where('deleted_at', '=', null)->exists()) {
          ...
          if ($Sale_data->branch_id) {
              if (branch::where('id', $Sale_data->branch_id)
                  ->where('deleted_at', '=', null)
                  ->first()) {
                  $sale['branch_id'] = $Sale_data->branch_id;
              } else {
                  $sale['branch_id'] = '';
              }
          } else {
          ...
          foreach ($Sale_data['details'] as $detail) {
              ...
              if ($detail->product_variant_id) {
                  $item_product = product_branch::where('product_id', $detail->product_id)
                      ...
                      ->where('branch_id', $Sale_data->branch_id)
                      ->first();
                  ...
                  $item_product = product_branch::where('product_id', $detail->product_id)
                      ->where('deleted_at', '=', null)->where('branch_id', $Sale_data->branch_id)
    ...
    public function Elemens_Change_To_Sale(Request $request, $id)
    {
        ...
        if ($Quotation->branch_id) {
            if (branch::where('id', $Quotation->branch_id)
                ->where('deleted_at', '=', null)
                ->first()) {
    ...
    public function get_Products_by_sale(Request $request , $id)
    {
        ...
        $Return_detail['branch_id'] = $SaleReturn->branch_id;
        ...
        foreach ($SaleReturn['details'] as $detail) {
            ...
            if ($detail->product_variant_id) {
                $item_product = product_branch::where('product_id', $detail->product_id)
                    ->where('product_variant_id', $detail->product_variant_id)
                    ->where('deleted_at', '=', null)
                    ->where('branch_id', $SaleReturn->branch_id)
                    ->first();
