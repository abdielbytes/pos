...
class SalesReturnController extends BaseController
{
    ...
    public function index(request $request)
    {
        ...
        $SaleReturn = SaleReturn::with('sale','facture', 'client', 'branch')
            ...
            foreach ($old_return_details as $key => $value) {
                ...
                if($value['sale_unit_id'] !== null){
                    if ($current_SaleReturn->statut == "received") {
                        if ($value['product_variant_id'] !== null) {
                            $product_branch = product_branch::where('deleted_at', '=', null)->where('branch_id', $current_SaleReturn->branch_id)
                        ...
                        if ($product_detail['product_variant_id'] !== null) {
                            ...
                            if ($unit_prod && $product_branch) {
                                if ($unit_prod->operator == '/') {
                                    $product_branch->qte += $product_detail['quantity'] / $unit_prod->operator_value;
                                } else {
                                    $product_branch->qte += $product_detail['quantity'] * $unit_prod->operator_value;
                                }
                                $product_branch->save();
    ...
    public function show(Request $request, $id)
    {
        ...
        $return_details['branch'] = $Sale_Return['branch']->name;
