          ...
          <b-col lg="12" md="12" sm="12">
            <b-card>
              <b-row>
                ...
                <!-- branch -->
                <b-col lg="4" md="4" sm="12" class="mb-3">
                  <validation-provider name="branch" :rules="{ required: true}">
                    <b-form-group slot-scope="{ valid, errors }" :label="$t('Branch') + ' ' + '*'">
                      <v-select
                        :class="{'is-invalid': !!errors.length}"
                        :state="errors[0] ? false : (valid ? true : null)"
                        :disabled="details.length > 0"
                        @input="Selected_branch"
                        v-model="sale.branch_id"
                        :reduce="label => label.value"
                ...
                <b-col md="4" v-if="payment.status != 'pending'">
                  ...
                  <b-col md="4" v-if="payment.status != 'pending'">
                      <validation-provider
                        ...
                        v-slot="validationContext"
                      >
                        <b-form-group :label="$t('Price Of Product') + ' ' + '*'">
                          <b-form-input
                ...
                <b-col md="4" v-if="payment.status != 'pending'">
                  ...
                  <!-- PaymentFrequency-->
                  <b-col lg="4" md="4" sm="12" class="mb-3" v-if="payment.status != 'pending'">
                      <validation-provider name="payment_frequency">
                          <b-form-group label="payment_frequency">
                              <v-select
                                  v-model="sale.payment_frequency"
                                  :reduce="label => label.value"
                  ...
                  <p>Note: If payment is in installments defaulters will be charged a penalty each time they default payment.</p>
              ...
              >
                <b-form-group :label="$t('ProductPrice') + ' ' + '*'" id="Price-input">
                  <b-form-input
                    ...
                    :state="getValidationState(validationContext)"
                    aria-describedby="Price-feed back"
                  ></b-form-input>
...
export default {
  ...
  methods: {
    ...
    Modal_Updat_Detail(detail) {
      ...
      this.detail.payment_frequency = detail.payment_frequency;
       setTimeout(() => {
        NProgress.done();
    ...
    search(){
      ...
      if (this.sale.branch_id != "" &&  this.sale.branch_id != null) {
    ...
    Selected_branch(value) {
      this.search_input= '';
      this.product_filter = [];
      this.Get_Products_By_branch(value);
    },

     //------------------------------------ Get Products By branch -------------------------\\

    Get_Products_By_branch(id) {
      // Start the progress bar.
        NProgress.start();
        NProgress.set(0.1);
      axios
        .get("get_Products_by_branch/" + id + "?stock=" + 1 + "&is_sale=" + 1 + "&product_service=" + 1)
         .then(response => {
        ...
        if (this.payment.Reglement == "credit card" && this.is_new_credit_card) {
          ...
          axios
            .post("sales", {
              ...
              statut: this.sale.statut,
              notes: this.sale.notes,
              payment_frequency: this.sale.payment_frequency,
              tax_rate: this.sale.tax_rate?this.sale.tax_rate:0,
              TaxNet: this.sale.TaxNet?this.sale.TaxNet:0,
              discount: this.sale.discount?this.sale.discount:0,
              shipping: this.sale.shipping?this.sale.shipping:0,
