        ...
        prevLabel: 'prev',
            ...
            >
                ...
                <div slot="table-actions" class="mt-2 mb-3">
                    ...
                    <b-button variant="outline-info ripple m-1" size="sm" v-b-toggle.sidebar-second>
                        <i class="i-installment-2"></i>
                        {{ $t("Edit Installment") }}
                    </b-button>
...
<!-- side bar Installments -->
<b-sidebar id="sidebar-second" :title="$t('Installments')" bg-variant="white" second shadow>
    <div class="px-3 py-2">
         <!-- Reference -->
         <b-col md="12">
            <b-form-group :label="$t('Installment')">
                <b-form-input label="Set Installment"  v-model="installment"></b-form-input>
            </b-form-group>
        </b-col>
        <b-col md="6" sm="12">
                        <b-button @click="Set_Installment()" variant="danger ripple btn-block m-1" size="sm">
                            <i class="i-Power-2"></i>
                            {{ $t("Set") }}
                        </b-button>
        </b-col>
...
<!--Modal Add penalty-->
        <validation-observer ref="Add_Penalty">
            <b-modal
                hide-footer
                size="lg"
                id="Add_Penalty"
                :title="EditPaiementMode?$t('EditPayment'):$t('AddPayment')"
            >
                <b-form @submit.prevent="Submit_Penalty">
                    <h1 class="text-center mt-3 mb-3">{{client_name}}</h1>
                    <b-row>
                        <!-- date -->
                        <b-col lg="4" md="12" sm="12">
                        ...
                        <b-col lg="4" md="12" sm="12">
                            ...
                            >
                                <b-form-group :label="$t('Paying_Amount')">
                                    <b-form-input
                                        @keyup="Verified_paidAmount(penalty.montant)"
                                        label="Amount"
                                        :placeholder="$t('Penalty_Amount')"
                                        v-model.number="penalty.montant"
                                        :state="getValidationState(validationContext)"
                                        aria-describedby="Amount-feedback">
<!--                                        :disabled="EditPaiementMode && payment.Reglement == 'credit card'"-->
                                    </b-form-input>
                ...
                <div style="max-width:400px;margin:0px auto">
                    <div class="info" >
                        ...
                        <p>
                            ...
                            <span v-show="pos_settings.show_branch">{{$t('branch')}} : {{invoice_pos.sale.branch_name}} <br></span>
                        </p>
                    </div>
    ...
    methods: {
        ...
        onSortChange(params) {
            ...
            if (params[0].field == "client_name") {
                field = "client_id";
            } else if (params[0].field == "branch_name") {
                field = "branch_id";
        ...
        Submit_Payment() {
            this.$refs.Add_payment.validate().then(success => {
                ...
                else if(this.payment.montant < this.sale.installment[0] ){
                    this.makeToast(
                        "warning",
                        this.$t("Paying_amount_is_less_than_agreed_installments"),
                        this.$t("Warning")
                    );
                    this.payment.montant = 0;
        ...
        //     this.installment = "";
        // },
        async Set_Installment() {
        // Assuming `installment` is the updated value and `installmentId` is the ID
        const payload = {
            installment: this.installment
        };

        try {
            await axios.put(`/installments/1`, payload); // Replace `1` with the actual ID if dynamic
            this.$bvToast.toast('Installment updated successfully', {
                title: 'Success',
                variant: 'success',
                solid: true
            });
        } catch (error) {
            this.$bvToast.toast('Failed to update installment', {
                title: 'Error',
                variant: 'danger',
                solid: true
            });
    ...
    //   },
        ...
        setToStrings() {
            ...
            if (this.Filter_Client === null) {
                this.Filter_Client = "";
            } else if (this.Filter_branch === null) {
                this.Filter_branch = "";
            ...
            // }else{
                axios
                    .post("penalty_sale", {
                        sale_id: this.sale.id,
                        date: this.payment.date,
                        penalty_amount: parseFloat(this.penalty.montant).toFixed(2),
                        // received_amount: parseFloat(this.payment.received_amount).toFixed(2),
                        // change: parseFloat(this.payment.received_amount - this.payment.montant).toFixed(2),
                        // Reglement: this.payment.Reglement,
