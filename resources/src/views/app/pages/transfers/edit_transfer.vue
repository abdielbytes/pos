          ...
          <b-col lg="12" md="12" sm="12">
            <b-card>
              <b-row>
                ...
                <!-- From branch -->
                <b-col lg="4" md="4" sm="12" class="mb-3">
                  <validation-provider name="From branch" :rules="{ required: true}">
                    <b-form-group slot-scope="{ valid, errors }" :label="$t('Frombranch') + ' ' + '*'">
                      <v-select
                        :class="{'is-invalid': !!errors.length}"
                        :state="errors[0] ? false : (valid ? true : null)"
                        :disabled="details.length > 0"
                        @input="Selected_From_branch"
                        v-model="transfer.from_branch"
                ...
                <!-- To branch -->
                <b-col lg="4" md="4" sm="12" class="mb-3">
                  <validation-provider name="To branch" :rules="{ required: true}">
                    <b-form-group slot-scope="{ valid, errors }" :label="$t('Tobranch') + ' ' + '*'">
                      <v-select
                        :class="{'is-invalid': !!errors.length}"
                        :state="errors[0] ? false : (valid ? true : null)"
                        v-model="transfer.to_branch"
                        :reduce="label => label.value"
                        :placeholder="$t('Choose_branch')"
                        :options="branches.map(branches => ({label: branches.name, value: branches.id}))"
                      />
...
export default {
  ...
  // Search Products
    search(){
      ...
      if (this.transfer.from_branch != "" &&  this.transfer.from_branch != null) {
    ...
    verifiedForm() {
      if (this.details.length <= 0) {
        ...
        return false;
      } else if (this.transfer.from_branch === this.transfer.to_branch) {
        this.makeToast(
          "warning",
          this.$t("branchIdentical"),
          this.$t("Warning")
        );
        return false;
      } else {
    ...
    Get_Products_By_branch(id) {
      // Start the progress bar.
        ...
        NProgress.set(0.1);
      axios
        .get("get_Products_by_branch/" + id + "?stock=" + 1 + "&product_service=" + 0)
         .then(response => {
    ...
    //---------------------- Event Select From branch ------------------------------\\
    Selected_From_branch(value) {
      this.search_input= '';
      this.product_filter = [];
    ...
    GetElements() {
      ...
      axios
        ...
        .then(response => {
          ...
          this.Get_Products_By_branch(this.transfer.from_branch);
          this.Calcul_Total();
